...
# Verificar versión mínima requerida (Go 1.16+ para io/fs, pero el proyecto requiere 1.25.8)
if [ "$GO_MAJOR" -lt 1 ] || ([ "$GO_MAJOR" -eq 1 ] && [ "$GO_MINOR" -lt 16 ]); then
    error "Go versión $GO_VERSION detectada. Se requiere Go 1.16 o superior (el proyecto requiere Go 1.25.8)"
    echo ""
    ...
    echo "  3. Usar Docker: docker run --rm -v \$(pwd)/backend:/app -w /app golang:1.25.8-alpine3.22 go test ./..."
    exit 1
elif [ "$GO_MAJOR" -eq 1 ] && [ "$GO_MINOR" -lt 25 ]; then
    warning "Go versión $GO_VERSION detectada. Se recomienda Go 1.25.8 o superior"
...
if go mod download 2>&1 | tee /tmp/gomod_output.txt; then
    ...
    if grep -q "malformed module path\|cannot load\|missing dot" /tmp/gomod_output.txt; then
        ...
        error "Se requiere Go 1.16+ (recomendado: Go 1.25.8+)"
        echo ""
        echo "Soluciones:"
        echo "  1. Actualizar Go: https://go.dev/dl/"
        echo "  2. Usar g: go install github.com/voidint/g@latest && g install 1.25.8"
...
if [ "$RUN_VET" = true ]; then
    ...
    if go vet $VET_PATH 2>&1 | tee /tmp/govet_output.txt; then
        ...
        if grep -q "malformed module path\|cannot load\|missing dot" /tmp/govet_output.txt; then
            ...
            warning "Se requiere Go 1.16+ (recomendado: Go 1.25.8+)"
            echo ""
