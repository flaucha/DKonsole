...
if [ "$SKIP_TESTS" = false ]; then
    ...
    if ! command_exists golangci-lint || [[ "$(golangci-lint version 2>&1)" != *"go$(go version | grep -oP 'go\d+\.\d+')"* ]]; then
        ...
        GOTOOLCHAIN=go1.25.5 go install github.com/golangci/golangci-lint/cmd/golangci-lint@v1.64.8
    fi
    ...
    if command_exists govulncheck; then
        ...
        GOTOOLCHAIN=go1.25.5 go install golang.org/x/vuln/cmd/govulncheck@latest
